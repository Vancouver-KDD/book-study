...
> In Chapter 8 and Chapter 9: Node failures; Unreliable networks; and trade-offs around replica consistency, durability, availability, and latency

### Implementation of Replication Logs
Four different replication methods

#### (1) Statement-based replication
  - Leader logs every write request (statement) and sends that statement logs to followers.
  - For a RDBS, all INSERT, UPDATE, or DELETE statement ==> followers
  - Disorder cases
     - Nondeterministic function(i.e. NOW(): current date and time, RAND(): random number) ==> different value on each replica.
     - Autoincrementing column or Depending on the existing data (e.g. UPDATE … WHERE <some condition>), they must be executed in exactly the same order on each replica, or else they may have a different effect.
...
- Disadvantage
 - the log is very *Low level data* & *Details of changed bytes* for the disk blocks.
 - Closely coupled to the storage engine. ==> not possible to run different versions of the database software
 - Down time is needed for Upgrade
   - If Write-ahead log (WAL) is allow followers to use a newer software version than the leader ==> Zero-downtime upgrade of the database software.

#### (3) Logical (row-based) log replication
- For decoupling of the storage engine internals: *different log formats* for *replication* and for *storage engine*
- A logical log for a relational database: a sequence of records describing writes to database tables at a row:
  - Inserted row: the log contains the new values of all columns.
  - Deleted row: the log contains enough information to uniquely identify the row that was deleted.
  ...
  - Updated row: the log contains enough information to uniquely identify the updated row, and the new values of all columns
    - or at least the new values of all columns that changed.
- A modification transaction for several rows generates several such log records, followed by a record indicating that the transaction was committed.
  - MySQL’s binlog (when configured to use row-based replication) uses this approach.
- Good backward compatible: because it's decoupled.
...
- Easier for external applications to parse.
  - useful to send the contents of a database to an external system (data warehouse for offline analysis / building custom indexes and caches (change data capture in Ch.11))

#### (4) Trigger-based replication
- Replication logs using application code for more flexibility at the application layer
  - i.e., if you want to only replicate a subset of the data,
  - or want to replicate from one kind of database to another,
  - or if you need conflict resolution logic
- Some tools, such as Oracle GoldenGate, can make data changes available to an application by reading the database log.
- An alternative is to use RDBS features such as triggers and stored procedures.
  - it automatically executes when a data change (write transaction) occurs in a database system.
  - The trigger has the opportunity to log this change into a separate table, from which it can be read by an external process  - Databus for Oracle and Bucardo for Postgres
    - (That external process can then apply any necessary application logic and replicate the data change to another system)
- But, greater overheads / more bugs / more limitations comparing to database’s built-in replication.
- Nevertheless, flexibility ==> useful


## Problems with Replication Lag
We want Replication. why? 
1. Tolerability for node failures
...
- Read-scaling architecture
  - in the Leader-based replication
    - All writes ==> a single leader node
    - Read-only queries ==> any replica.
- For workloads that consist of mostly reads
  - Many followers, and distribute the read requests across those followers.
  - This removes workload simply by adding more followers
...
- Asynchronous replication....
  - Maybe Out‐dated information(inconsistency): if the follower has fallen behind
  - Different results on the leader and a follower from the same query - because not all writes have been reflected in the follower
...
- Especially appropriate ==> more read & less write.
...
- Case:
  - User writes data ==> User shortly views data ==>  The new data not yet applied to the replica.
  - To the user, it looks lost and unhappy.
- *read-after-write consistency*(a.k.a. *read-your-writes consistency*)
...
- (Figure 5-4): refreshing a web page (Each request ==> to a random server)
- User 2345 makes the same query twice
  - first to a follower 1 with little lag ==> 1 result
...
- However, if that replica fails, then the user can see the disappear again.
